cat("((((Strix_aluco:4.2,Asio_otus:4.2):3.1,",
    "Athene_noctua:7.3):6.3,Tyto_alba:13.5):7.1,pancho:15.3);",
    file = "ex.tre", sep = "\n")
...
plot(tree.owls, main = "With branch lengths",show.tip.label = FALSE,show.node.label = TRUE,edge.lty = c(2,1,1,1,2))
...
wh = which.edge(bird.orders, 15:24)
dot = rep(1,Nedge(bird.orders))
dot[wh] = 2
plot(bird.orders, show.tip.label = F, use.edge.length = F,edge.lty = dot,main = "Phylogenetic tree")
#rect(1.2, 0.5, 22, 5.4, col = "orange")
rect(20.8, 0.7, 22.3, 14.5, border = "green")
par(new=TRUE)
plot(bird.orders, show.tip.label = F, use.edge.length = F,edge.lty = dot)
rect(-0.5, 0.5, 22.6, 23.5, border = "blue")
rect(13.7, 14.4, 19.8, 23.3, border = "red")
